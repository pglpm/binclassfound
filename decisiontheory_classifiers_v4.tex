...
%% Last-Updated: 2022-05-26T20:07:46+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}\labelsep 0pt%
  ...
  How can we measure something, if our meter does not follow any standard and expands or shrinks depending on the measured object? The present work argues that current evaluation and comparison practices for \ml\ classifiers are affected by exactly this kind of problems; the correctness of an evaluation often relies more on luck than on principles. Negative consequences then appear when classifiers are put to real use. It proposes to ground evaluation on Decision Theory. The main result is that any evaluation metric must be a linear combination of the confusion-matrix elements, with coefficients that depend on the specific classification problem. For binary classification, the space of such possible metrics is effectively two-dimensional. It is shown that popular metrics such as precision, balanced accuracy, Matthews Correlation Coefficient, Fowlkes-Mallows index, and $F_{1}$-measure do not satisfy basic decision-theoretic principles and always lead to incorrect rankings, in every classification problem. They 
...
\end{equation}
These values might arise in several scenarios. For example, tumours of class~0 and~1 may require very different treatments, say surgical and drug-based. If a class~0 tumour is misdiagnosed and not properly treated, it leads to immediate death (0\,months); if correctly diagnosed, its treatment is usually successful, leading to high life expectancy (500\,months). Class~0 tumours can be treated, but they lead to a shorter life expectancy (350\,months). If they are misdiagnosed as class~1, however, the damage caused by class~1 treatment somewhat shortens this life expectancy (300\,months).

This matrix above is numerically equivalent to~\eqref{eq:utility_example} up to a common additive constant of $335$, so the final net gains are also simply shifted by this amount. The value 0 means immediate death. It is easy to see that the metrics are exactly as in Table~\ref{tab:example_metrics}, the majority favouring classifier~$\Beta$. And yet the use of classifier~$\Alpha$ leads to a more than six-month longer expected remaining life than classifier~$\Beta$.
...
\end{equation}
With this convention, it is clear that we only have two degrees of freedom in choosing the utility matrix of a binary classification. As a consequence, \emph{the space of possible evaluation metrics for binary classification is two-dimensional}. In order to evaluate candidate classifiers for a binary classification we must choose a point from this space.
...
\caption{Relationship between various evaluation metrics and actual utility yields for a  two different binary classifications with underlying utility matrices $\begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix}$ (left column) and $\begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix}$ (right column). All confusion matrices ({\color{mypurpleblue}blue dots}) are obtained from a dataset with 50\%/50\% class balance.
%
...
\item a uniform distribution; \item a bivariate (truncated) gaussian distribution centred on the identity matrix $\begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix}$ and with standard deviation $1/3$ in the $x$ and $y$ coordinates of \eqn~\eqref{eq:coords_um}, illustrated in \fig~\ref{fig:gauss_distr_um}.
\end{enumerate*}
...
\begin{figure}[t]
  ...
  \parbox[t]{0.45\linewidth}{%
    ...
    \caption{Extents of errors having standard deviations $0.1$ ({\color{mypurpleblue}blue triangles}) and $0.2$ ({\color{myred}red squares}), around the utility matrices $\begin{bsmallmatrix} 0.5&0.5\\0&1 \end{bsmallmatrix}$ and $\begin{bsmallmatrix} 1&0\\0.5&0.5 \end{bsmallmatrix}$ (black diamonds).}
...
The theory, remarks, and results of the present work generalize beyond classification, to regression and more complex classification-like problems such as image segmentation. It would be interesting to examine whether popular metrics in the latter field, such as Dice score \autocites{dice1945,fleiss1975,zijdenbosetal1994} and Hausdorff distance \autocites{altetal2000}, comply with decision-theoretic principles, and which alternatives could be used otherwise.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\begin{contributions}
  All authors have contributed equally to the present work, especially KD.
\end{contributions}
