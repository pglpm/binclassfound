...
%% Last-Updated: 2022-04-13T11:56:35+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
According to decision theory, a classification problem requires the specification of a utility matrix. We saw in \sect~\ref{sec:dt_utility_yield} that the utility matrix should also be used in evaluating the decisions made by the classification algorithm in a test set with $N$ datapoints. The decisions made by the algorithm and corresponding true classes constitute the confusion matrix $(M_{dc})$, containing the number $M_{dc}$ of times we made decision $d$ when the true class was $c$. We saw that the total and average utilities obtained by the classifier algorithm on the test set are, in terms of the independent components of the confusion matrix,
\begin{equation}
...
\begin{itemize}
\item[\checkmark] \emph{Accuracy:} $(M_{\tp}-M_{\fp}+N_{\Ne})/N$. It is a particular case of formula~\eqref{eq:utility_testset_binary} with $x_{\tp}=y_{\Ne}=1$, $x_{\fp}=-1$, $y_{\Po}=0$. In fact it corresponds to using a utility matrix equivalent to the identity $\begin{psmallmatrix} 1&0\\0&1 \end{psmallmatrix}$.
  
\item[\texttimes] \emph{Precision:} $M_{\tp}/(M_{\tp}+M_{\fp})$.  It cannot be written as a linear expression in $M_{\tp},M_{\fp}$.

\item[\texttimes] \emph{$F_{1}$-measure:} $2 M_{\tp}/(M_{\tp} + M_{\fp} + N_{\Po})$.

\item[\texttimes] \emph{Matthews correlation coefficient:} $2 M_{\tp}/(M_{\tp} + M_{\fp} + N_{\Po})$.
\end{itemize}
