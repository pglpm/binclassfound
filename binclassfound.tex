...
%% Last-Updated: 2022-04-12T08:21:15+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
For \ml\ classifiers one typically considers situations where the set of available decisions and the set of possible classes have some kind of natural correspondence and equal in number. In a \enquote{cat vs dog} image classification, for example, the classes are \enquote{image of a cat} and \enquote{image of a dog}, and the decisions could be \enquote{put into folder Cats} vs \enquote{put into folder Dogs}. In a medical application the classes could be \enquote{ill} and \enquote{healthy} and the decisions \enquote{treat} vs \enquote{dismiss}. In the following when we speak of \enquote{classification} we mean a decision problem of this kind.
...
Any decision problem is therefore completely determined by a set of $\nd \cdot \nc$ utilities. Actually if we change the elements of the utility matrix of a decision problem by a common additive constant or by a common positive multiplicative constant, then that decision problem is the same, in the sense that we reach the same decision by maximizing expected utility, given the same probabilities. This is evident from \eqn~\eqref{eq:exp_utility}: all expected utilities will change by the same additive constant or the same positive factor, and therefore their ordering will not change. After all an additive constant only represents a change in the zero of our utility scale, and a positive multiplicative constant represents a change in its measurement unit. Such changes should not affect a decision problem; the fact that they do not is another example of the logical consistency of decision theory.

Every decision problem with $\nd$ decisions and $\nc$ classes is therefore uniquely characterized by\; $\nd \nc - 2$\; parameters (the set of such problems has the topology of an $(\nd\nc-1)$-dimensional sphere). In the case of binary classification this means 2 parameters.
