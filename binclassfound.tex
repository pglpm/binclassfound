...
%% Last-Updated: 2022-04-08T15:11:28+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
How does the probability calculus determine the probability $\p(X \| Z\mo z, D)$? Here is a simplified, intuitive picture. First consider the case where the possible values the feature $Z$ can assume are few, so that many units can in principle have the same value of $Z$.

Consider the collection of all units having $Z\mo z$ that we would receive in the future. Among them, a proportion $F(X\mo 0 \| Z\mo z)$ will belong to class $0$, and a proportion $1 - F(X\mo 0 \| Z\mo z) \equiv F(X\mo 1 \| Z\mo z)$ will belong to class $1$. For example the two proportions could be 74\% and 26\%.



If , then Such a collection is more difficult to imagine is the case where the feature $Z$ can assume a continuum of values -- as is the case for a brain scan, for example. In this case each unit will be unique in its feature value -- no two brains are exactly alike.
