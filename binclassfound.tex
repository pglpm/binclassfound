...
%% Last-Updated: 2022-04-11T16:05:10+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
All the issues above are manifestly connected: they involve considerations of importance or gain or cost, and of uncertainty.
...
To each decision we associate a \emph{utility} which depends on which of the possible classes is actually true. The utility may equal a concrete monetary gain or cost, or a more abstract value of importance.

For example, we are offered to buy a lottery ticket, which may be winning or not. The ticket costs $1$ unit of some monetary currency to buy, and the lottery prize is $101$ units. Our decisions are whether to buy the ticket or not. We have four utilities, represented in the following self-explanatory table:
\begin{center}
  \begin{tabular}{rcc}
    &\multicolumn{2}{c}{\small winning?}\\
    \cmidrule(lr){2-3}
    {\small buy?}&\texttt{yes}&\texttt{no}\\
    \cmidrule(lr){1-1}
    \texttt{yes}&+100&-1\\
    \texttt{no}&0&0
  \end{tabular}
...
We denote $\U(d\|c)$ the utility of decision $d$ if class $c$ is true, or  \enquote{the utility of $d$ given $c$}, or \enquote{the utility of $d$ conditional on $c$}
...
\end{equation}
where $\p(c_{1})$ and $\p(c_{2}) \equiv 1 - \p(c_{1})$ are the probabilities of classes $c_{1}$ and $c_{2}$.
