...
%% Last-Updated: 2022-04-13T14:36:05+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
We thus find the following important result according to decision theory: \emph{A valuation scale should be a \textbf{linear combination} of the elements of the confusion matrix, possibly multiplied by a positive function of the class frequencies of the test set and with an additional term also depending only on the class frequencies. The coefficients of the linear combination are problem-specific and \textbf{cannot depend on the confusion matrix}.}
...
\begin{equation*}
  ...
  U_{\Ne\Ne} \equiv U_{\tn} \ .
\end{equation*}
With this notation, decision theory says that a valuation scale for this test set should have the following general form
\begin{equation}
 ...
 x_{\fn}\, M_{\fn} + x_{\tn}\, M_{\tn}) +
  b(N_{\Po},N_{\Ne}) \ .
\end{equation}
for particular values of the coefficients $x_{\tp}, x_{\fp}, x_{\fn}, x_{\tn}$ and particular functions $a(\dotv)>0$ and $b(\dotv)$. Let us examine some common valuation scales according to this requirement.

\begin{itemize}
\item[\itemyes] \emph{Accuracy:} $(M_{\tp}-M_{\fp}+N_{\Ne})/N$. It is a particular case of formula~\eqref{eq:utility_testset_binary} with $x_{\tp}=x_{\tn}=1$, $x_{\fp}=x_{\fn}=01$, $a(N_{\Po},N_{\Ne})=1/N$, and $b(\dotv)=0$. It corresponds to using the utility matrix $\begin{psmallmatrix} 1&0\\0&1 \end{psmallmatrix}$.
...
\item[\itemno] \emph{Balanced accuracy:} $(N_{\Ne}\,M_{\tp} + N_{\Po}\, M_{\tn})/(2 N_{\Po}N_{\Ne})$. Despite being an average of two scales (true-positive and true-negative rate) that agree with formula~\eqref{eq:utility_testset_binary}, it is not an instance of that formula. The reason is that the two averaged scales involve different $a(\dotv)$ functions.
\end{itemize}
\mynote{\puzzle\ It seems most or all commonly used scales do not comply with decision theory!}
...
% We thus find the following important result according to decision theory: \emph{A valuation scale should be a \textbf{linear combination} of the independent elements of the confusion matrix, possibly normalized to the number of test data. The coefficients of the linear combination are problem-specific and \textbf{cannot depend on the confusion matrix}, nor on the frequencies of the classes.}
...
% A valuation scale consistent with decision theory must take one of the forms above for particular values of the coefficients $x_{\tp}, x_{\fp}, y_{\Po}, y_{\Ne}$. Let us examine some common valuation scales according to this requirement.
...
% \end{itemize}
% \mynote{\puzzle\ It seems most or all commonly used scales, except accuracy, do not comply with decision theory!}
