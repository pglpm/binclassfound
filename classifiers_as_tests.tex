...
%% Last-Updated: 2022-04-16T19:15:32+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{itemize}
The proportionality constant in \eqn~\eqref{eq:weight_F_two_factors} is simply the integral $\smallint F(D)\, \wfo(F)\, \di F$ ensuring that $\wf(F)$ is normalized.

Note that the first factor becomes larger as the number of known data increases. Thus a large amount of data indicating a non-smooth distribution $F$ will override any smoothness preferences embodied in the second factor.

\medskip

The expected value~\eqref{eq:prob_is_expe_freq} is numerically approximated by a sum over a sample of distributions $F$ having largest weights, obtained by Markov-chain Monte Carlo sampling. The error of this approximation can be calculated and made as small as required by increasing the number of Monte Carlo samples.
