...
%% Last-Updated: 2022-04-15T22:08:16+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
There are indeed many parallels in the way \ml\ classifiers and diagnostic tests, a flu test for example, are devised and work. For both, in principle we would like to assess some situation -- class, pathological condition -- by means of its correlation (which includes deterministic dependence as a limiting case) with a set of variables that is either very complex or not easily observable -- features, presence of replicating viral agents:
\[
  \text{situation} \leftrightarrow \text{complicated variables}
\]
We devise an auxiliary variable -- output, test result -- to be correlated with both the situation and the difficult set of variables:
\[
  \begin{aligned}
    \text{situation} &\leftrightarrow \text{complicated variables}
    \\[-\jot]
...
\\[-2.5\jot] &{}\:\text{aux variable}
  \end{aligned}
\]
so that we can now assess the situation by observing the more easily accessible auxiliary variable. The correlation of the auxiliary variable is achieved by the training process in the case of the \ml\ algorithm, and by exploiting biochemical processes or reactions in the case of the flu test.
