...
%% Last-Updated: 2022-05-19T10:27:05+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dealing with unknown utilities is straightforward. Suppose we are uncertain whether the utility matrix appropriate to a classification problem is $\uncu{1} \equiv \bigl(U^{(1)}_{ij}\bigr)$ or $\uncu{2}$ or $\uncu{3}$ and so on, where the number of alternatives can even be infinite or continuous. Each alternative $\uncu{a}$ has a probability $q_a$, or probability density $q(a)\ \di a$ in the continuous case. Then \emph{for the classification problem we should use the expected utility matrix
  \begin{equation}
    \label{eq:expe_UM}
    \mathte{U} \defd q_{1}\ \uncu{1} + q_{2}\ \uncu{2} + q_{3}\ \uncu{3} + \dotsb
  \end{equation}
  (or $\mathte{U} \defd \int q(a)\, \uncu{a}\, \di a$ in the continuous case)}.
...
\begin{equation}
  ...
  \equiv \sum_{j} \Bigl[\sum_{a} q_{a}\ U^{(a)}_{ij} \Bigr]\ p_{j}
\end{equation}
in which the average utility matrix~\eqref{eq:expe_UM} appears as the \enquote{effective} utility matrix to be used for the class-decision problem alone.
...
\subsection{Consequences of wrong utility assessments}
