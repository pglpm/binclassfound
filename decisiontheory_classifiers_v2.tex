...
%% Last-Updated: 2022-05-16T21:17:18+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The other three plots in the upper panel of \fig~\ref{fig:metrics_vs_utility} are analogous but for other metrics as $y$-coordinates. The lower panel is analogous to the upper panel, but for an imbalanced-dataset case, with one class appearing with $80\%$ frequency. Finally, \fig~\mynotep{} is analogous to \fig~\ref{fig:metrics_vs_utility} but with an underlying utility matrix equal to $ \begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix} $.
...
\begin{figure}[p]
  \centering
  %{A}\hfill\mbox{}\\[0pt]
\includegraphics[width=0.99\linewidth]{utility_vs_metrics_1001.jpg}\\[0pt]
% {B}\hfill\mbox{}\\[0pt]
\vspace{-1em}{\color{mygrey}\rule{0.66\linewidth}{0.25pt}}
\includegraphics[width=0.99\linewidth]{utility_vs_metrics_15-35-335165.jpg}\\
  \caption{Relationship between various metrics and utility metric for two classification problems with different utility matrices. Upper panel: classification with utility matrix $ \begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix} $. Lower panel: classification with utility matrix $ \begin{bsmallmatrix} 15 & -335  \\ -35 & 165 \end{bsmallmatrix} $, \eqn~\eqref{eq:utility_example}. Pairs of red triangles represent two confusion matrices -- the same two in all plots of one panel --  that are incorrectly ranked by the metrics reported on the $y$-axes.}
