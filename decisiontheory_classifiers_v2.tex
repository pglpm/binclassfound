...
%% Last-Updated: 2022-05-20T19:31:58+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\mynotew{add discussion of average over classification problems}
...
The area under the curve can only be computed for classifiers that output a continuous variable rather than a class. A threshold for this variable determines whether its value predicts of one class or the other. Different choices of threshold leads to different pairs of false-positive rate (which is $1-{}$true-negative rate) and true-positive rate in a given test set. These pairs can be plotted as a curve on a graph with corresponding axes. Given the proportion of classes in the test set, every point on such curve corresponds to a possible confusion matrix $C_{ij}(\theta)$ that the classifier can produce depending on the threshold $\theta$ chosen. The area subtended by such curve is a weighted average of true-positive rates with a peculiar choice of weights; the weights are uniform as a function of false-positive rate, but generally not uniform as a function of the threshold $\theta$. The meaning and proper use of the receiver operating characteristic are discussed in a classic by \textcites{metz1978}; see especially p.~290 and the section \emph{Cost/Benefit Analysis}, p.~295, which echoes some of the remarks below.

The following remarks can be made from the standpoint of decision theory; they share many points with the discussions by \textcites{bakeretal2001}, \textcite{loboetal2008}, and Metz cited above.

First, according to the principle of maximum expected utility, \sect~\ref{sec:dt_utilities}, the threshold $\theta$ should be chosen such as to maximize the utility yield, given by \eqn~\eqref{eq:final_utility}:
\begin{equation}
  \label{eq:threshold_auc_maxutility}
  \text{choose threshold}\quad
  \theta^{*} = \argmax_{\theta}\set[\bigg]{\sum_{i,j=0}^{1} U_{ij}\ C_{ij}(\theta)} \ .
\end{equation}
Any other values of the threshold are irrelevant. Averages over threshold values are therefore irrelevant as well.

Second, if we want an average of the performance over several classification problems, the quantities to be averaged are the utility matrices of those classification problems, as discussed in \sect~\ref{sec:unknown_utilities}, giving an expected utility matrix. Once this is computed we go back to a unique choice of threshold according to the first remark above.
...
Owing to the issues above, the area under the curve suffers from the same problems as the non-compliant metrics discussed in \sect~\ref{sec:common_metrics}: they always lead to some cases of incorrect evaluation, in every classification problem.

Denoting the class balance by $B$ and the true-positive and false-positive rates by $t$ and $f$, the confusion matrix can be written
\begin{equation}
