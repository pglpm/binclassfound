...
%% Last-Updated: 2022-05-19T23:24:42+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[b]
  ...
  \parbox[t]{0.33\linewidth}{%
    \centering\includegraphics[width=\linewidth]{distr_true_um_norm.pdf}\\
\caption{Truncated gaussian distribution in the space of utility matrices of \fig~\ref{fig:space_UM}.}\label{fig:gauss_distr_um}}
  \hspace{\stretch{1}}
  \parbox[t]{0.33\linewidth}{%
    \centering\includegraphics[width=\linewidth]{error_around_um.pdf}
  \caption{Extent of error ({\color{myredpurple}purple dots}) with standard deviation $0.1$, around the utility matrix $\begin{bsmallmatrix} 0.5&0.5\\0&1 \end{bsmallmatrix}$ (black diamond) in the space of utility matrices.}\label{fig:error_distr_um}}
  \hspace{\stretch{1}}
\end{figure}
...
\item\label{item:draw_um} Draw a \enquote{true} utility matrix.
\item\label{item:draw_wrongum} Draw some errors  around the elements of the true utility matrix, and add them to it.
\item Draw a class proportion.
\item\label{item:draw_cm} Draw two confusion matrices having that class proportion (the class proportion must be the same since the matrices are obtained from the same data).
