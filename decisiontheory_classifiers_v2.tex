...
%% Last-Updated: 2022-05-18T12:08:48+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[p]
  ...
  % {A}\hfill\mbox{}\\[0pt]
  \vspace{-0.5\headsep}
\includegraphics[width=0.95\linewidth]{utility_vs_metrics_0.5.jpg}\\[-1\baselineskip]
% {B}\hfill\mbox{}\\[0pt]
% \vspace{-1em}{\color{mygrey}\rule{0.66\linewidth}{0.25pt}}
% \includegraphics[width=0.95\linewidth]{utility_vs_metrics_0.8.jpg}\\[0pt]
\caption{Relationship between various evaluation metrics and actual utility yields for two binary-classification problems with underlying utility matrices $\begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix}$ (left column) and $\begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix}$ (right column). The dataset from which all confusion matrices are obtained has 50\%/50\% class balance.
%
  Pairs of {\color{myyellow}yellow non-triangular shapes} in a plot show two confusion matrices which are wrongly ranked by the metric of the plot's y-axis with respect to the actual utility yield (x-axis). Clearly there can  even be three or more confusion matrices ranked in completely opposite order by the metric.
%
  {\color{myred}Red triangular pairs} (almost overlapping in some plots) show two confusion matrices which are wrongly ranked by \emph{all} metrics but one simultaneously.
  %%
...
We see that the accuracy always gives correct relative evaluations of all confusion matrices when the underlying utility matrix is equivalent to $\begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix}$ (left column): the y-coordinates are a monotonically increasing function of the x-coordinates. Accuracy is indeed the utility yield corresponding to the identity utility matrix. The true-positive rate always gives correct relative evaluations when the underlying utility matrix is is equivalent to $\begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix}$ (right column) and the same test set is used for the evaluation.

On the other hand,



In all these plots we see that it is always possible to find two or more confusion matrices (pairs of yellow non-triangular shapes) for which the $F_{1}$-measure gives completely reversed evaluations with respect to the actual utility yield, scoring the worst confusion matrix -- and thus its associated algorithm -- as the best, and the best as the worst. The same holds for the Matthews correlation coefficient. It is even possible to find at least two confusion matrices (pairs of red triangular shapes) for which both these metrics give the wrong evaluation simultaneously. It turns out that these two metrics \emph{always lead to some incorrect evaluations in all  classification problem}. This is the second phenomenon mentioned above.

\mynotep{add comment about illogical to say that MCC is almost good in the first case}
...
\begin{figure}[t]
  \centering
    \includegraphics[width=\linewidth]{draws_0.jpg}\\
