...
%% Last-Updated: 2022-05-19T12:48:35+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
\item Two possible distribution of utility matrices on the set of \fig~\ref{fig:space_UM} (in that coordinate system): a uniform one, and a bivariate gaussian one centred on the identity matrix $\begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix}$ and with $0.5$ standard deviation in the $x$ and $y$ coordinates of \eqn~\eqref{eq:coords_um}.
...
\item Two possible distributions of error around a true utility matrix, represented by a bivariate gaussian centred on the true utility matrix and with standard deviations in the $x$ and $y$ coordinates equal to either $0.1$ or $0.25$.
\end{itemize}

Under these assumptions we calculate how often a pair of classifiers, represented by two confusion matrices having the same class proportions, leads to a reversed comparative evaluation with respect to the underlying utility. This is an integration problem that we solve by Monte Carlo sampling; the procedure is intuitive:
\begin{enumerate}[label=\arabic*.]
\item draw a u
\end{enumerate}
