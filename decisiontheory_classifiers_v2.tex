...
%% Last-Updated: 2022-05-15T12:19:25+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{quote}
  ...
  \begin{equation}
    ...
    a(N, f_{0}, f_{1})\ x\ C_{00} +
    a(N, f_{0}, f_{1})\ y\  C_{11} +
    b(N, f_{0}, f_{1})
  \end{equation}
  \emph{where $x,y$ are constants that do not depend on $C_{00}, C_{11}, N, f_{0}, f_{1}$, and $a(\dotv)>0$, $b(\dotv)$ are arbitrary functions of $N, f_{0}, f_{1}$ only}.
\end{quote}
...
\begin{itemize}
\item[\itemyes] \emph{Accuracy:} $C_{00}+C_{11}$. It is a particular case of formula~\eqref{eq:general_valuation_metric} with $a=1$, $x=y=1$, $b=0$. Indeed it corresponds to the evaluation metric based on the utility matrix $(U_{ij}) = \begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix}$.
  
\item[\itemyes] \emph{True-positive rate (recall):} $C_{00}/f_{0}$. It is  formula~\eqref{eq:general_valuation_metric} with $a=1/f_{0}$, $x=1$, $y=0$, $b=0$. It corresponds to using the utility matrix $\begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix}$. 

\item[\itemyes] \emph{True-negative rate (specificity):} $C_{11}/f_{1}$. We have $a=1/f_{1}$, $x=0$, $y=1$, $b=0$. It corresponds to using the utility matrix $\begin{bsmallmatrix} 0&0\\0&1 \end{bsmallmatrix}$. 
  
\item[\itemno] \emph{Precision:} $C_{00}/(C_{00}-C_{11}+f_{1})$. It cannot be written in the form~\eqref{eq:general_valuation_metric}; in fact it is non-linear in $C_{00}, C_{11}$.

\item[\itemno] \emph{$F_{1}$-measure:} $2 C_{00}/(C_{00} - C_{11} + 1)$. It cannot be written in the form~\eqref{eq:general_valuation_metric}; in fact it is non-linear in $C_{00}, C_{11}$. This also holds for the more general $F_{\beta}$-measures.
...
\item[\itemno] \emph{Matthews correlation coefficient:} $\frac{
    ...
    }}$. Non-linear in $C_{00}, C_{11}$, it cannot be written in the form~\eqref{eq:general_valuation_metric}.

\item[\itemno] \emph{Fowlkes-Mallows index:} $C_{00}/\sqrt{
      f_{0}\ (f_{1} + C_{00} - C_{11})}$. Also non-linear in $C_{00}, C_{11}$, it cannot be written in the form~\eqref{eq:general_valuation_metric}.

  \item[\itemno] \emph{Balanced accuracy:} $C_{00}/(2 f_{0}) + C_{11}/(2 f_{1})$. Despite being an average of two metrics (true-positive and true-negative rate) that are instances of formula~\eqref{eq:general_valuation_metric}, it is not an instance of that formula, because the two averaged metrics involve different $a(\dotv)$ functions.
\end{itemize}

We see that many popular validation metrics, those marked \enquote{\itemno}, do not comply with the principles of decision theory. As discussed in \sect~\ref{sec:decision_theory} this means that their formulae involve an interdependence of utilities and classification frequencies which implies some form of cognitive bias.

\autocites{handetal2018}
