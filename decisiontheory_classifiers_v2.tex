...
%% Last-Updated: 2022-05-20T18:39:25+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The area under the curve can only be computed for classifiers that output a continuous variable rather than a class. A threshold for this variable determines whether its value predicts of one class or the other. Different choices of threshold leads to different pairs of false-positive rate (which is $1-{}$true-negative rate) and true-positive rate in a given test set. These pairs can be plotted as a curve on a graph with corresponding axes. Given the proportion of classes in the test set, every point on such curve corresponds to a possible confusion matrix $C_{ij}(\theta)$ that the classifier can produce depending on the threshold $\theta$ chosen. The area subtended by such curve is a weighted average of true-positive rates with a peculiar choice of weights; the weights are uniform as a function of false-positive rate, but generally not uniform as a function of the threshold $\theta$.

The following remarks can be done
from the standpoint of decision theory.

First, according to the principle of maximum expected utility, \sect~\ref{sec:dt_utilities} \eqn~\eqref{eq:max_expe_utility}, the threshold $\theta$ should be chosen such as to maximize the utility yield, given by \eqn~\eqref{eq:final_utility}:
\begin{equation}
  \label{eq:threshold_auc_maxutility}
  \text{choose threshold}\quad
  \theta^{*} = \argmax_{\theta}\set[\bigg]{\sum_{ij} U_{ij}\ C_{ij}(\theta)} \ .
\end{equation}
Any other values of the threshold are irrelevant. Averages over threshold values are therefore irrelevant as well.

Second, if we want an average of the performance over several classification problems, the quantity to be averaged is the utility matrix of those classification problems, as discussed in \sect~\ref{sec:unknown_utilities}. Once this average is computed we go back to a unique choice of threshold according to the first remark above.
