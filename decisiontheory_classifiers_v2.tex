...
%% Last-Updated: 2022-05-07T14:31:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The manager of a factory which produces some kind of electronic component wishes to employ a machine-learning classifier to assess the durability of each component, which determines whether the component will be used in one of two possible kinds of device. The classifier takes some complex features of an electronic component as input, and outputs one of the two labels \enquote{0} for \enquote{short durability}, or \enquote{1} for \enquote{long durability}, depending on the forecast durability.

Two candidate classifiers, let us call them A and B, are trained on available training data. When employed on an evaluation set they yield the following confusion matrices, written in a
\begin{equation*}
...
\end{equation*}
format and normalized over the total number of evaluation data:
\begin{align}
  \label{eq:CM_A}
\text{\small classifier A:}\quad  \begin{bmatrix}
    0.39 & 0.14 \\ 0.31 & 0.16
  \end{bmatrix}
\ ,
  \\
  \label{eq:CM_B}
\text{\small classifier B:}\quad  \begin{bmatrix}
    0.38 & 0.00 \\ 0.32 & 0.30
  \end{bmatrix}
\ .
\end{align}
...
\begin{align*}
  \text{classifier A:}& \quad 3.2\,\text{\texteuro/component,} \\
  \text{classifier B:}& \quad -4.0\,\text{\texteuro/component.}
\end{align*}
That is, classifier B actually led to a \emph{loss} of revenue. The manager therefore decides to employ classifier A, commenting with a smug smile that the developers' recommendation wasn't very good.
...
Taking the sum of the products of the gains above by the respective percentages of occurrence -- which are the elements of a confusion matrix -- gives the final average gain. In the present case, the confusion matrices~\eqref{eq:CM_A} and \eqref{eq:CM_B} lead indeed to the amounts found by the manager.
