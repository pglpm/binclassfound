...
%% Last-Updated: 2022-05-11T15:32:10+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Current issues in evaluation of classifiers}
\label{sec:intro}
...
\begin{quotation}
\small
  The manager of a factory which produces some kind of electronic component wishes to employ a machine-learning classifier to assess the durability of each component, which determines whether the component will be used in one of two possible kinds of device. The classifier should take some complex features of an electronic component as input, and output one of the two labels \enquote{0} for \enquote{short durability}, or \enquote{1} for \enquote{long durability}, depending on the component type.
...
\begin{align}
  \label{eq:CM_A}
\text{classifier A:}\quad  \begin{bmatrix}
    0.39 & 0.14 \\ 0.31 & 0.16
  \end{bmatrix}
\ ,
  \\
  \label{eq:CM_B}
\text{classifier B:}\quad  \begin{bmatrix}
    0.38 & 0.00 \\ 0.32 & 0.30
  \end{bmatrix}
\ .
\end{align}
...
\begin{center}\footnotesize
  \begin{tabular}{lcc}
    ...
    \hline
    Accuracy & \bad{0.55} & \good{0.68} \\
    Precision & \bad{0.74} & \good{1.00} \\
    F1 measure & \bad{0.63} & \good{0.70} \\
    Matthews correlation coefficient & \bad{0.08} & \good{0.51} \\
    Balanced accuracy & \bad{0.55} & \good{0.77} \\
    True-positive rate (recall) & \good{0.56} & \bad{0.54} \\
    True-negative rate (specificity) & \bad{0.53} & \good{1.00}
  \end{tabular}
\end{center}
The majority of these metrics favours classifier B, some of them by quite a wide margin. Only the true-positive rate favours classifier A, but only by a relative difference of less than 3\%. Indeed classifier B perfectly classifies the long-durability components, and neither classifier is especially good at classifying the short-durability ones. The developers of the classifiers therefore recommend the employment of classifier B.
...
During the trial period the classifiers perform according to the classification statistics of the confusion matrices~\eqref{eq:CM_A} and \eqref{eq:CM_B} above. At the end of this period the factory manager finds that the average net gains per assessed component brought about by the two classifiers are
\begin{equation}
  \label{eq:final_gains}
\begin{split}
  \text{classifier A:}& \quad \good{3.2}\,\text{\texteuro/component}\ , \\
  \text{classifier B:}& \quad \bad{{-4.0}}\,\text{\texteuro/component}\ .
\end{split}
\end{equation}
That is, classifier B actually led to a \emph{loss} of revenue. The manager therefore decides to employ classifier A, commenting with a smug smile that it was wise not to trust the developers' recommendations.
...
\begin{equation}
  ...
  \rotatebox[origin=c]{90}{
    ...
    \overbracket[0pt]{
      \begin{bmatrix}
        340\,\text{\texteuro} & 240\,\text{\texteuro}  \\
        -660\,\text{\texteuro} & 260\,\text{\texteuro}
      \end{bmatrix}}^{
      \textit{\parbox{\widthof{\scriptsize true class}}{\centering\scriptsize true class\\$0\hspace{3em}1$}}
    }
\end{equation}
The reason behind these values is that short-durability components cause extreme damage (and consequent repair costs and refunds) if used in devices that require long-durability components. Devices that only require short-durability components, on the other hand, also work well with long-durability ones, although slightly less efficiently.

Taking the sum of the products of the gains above by the respective percentages of occurrence -- which are the elements of the confusion matrix -- yields the final average gain. In the present case, the confusion matrices~\eqref{eq:CM_A} and \eqref{eq:CM_B} lead to the amounts \eqref{eq:final_gains} found by the manager.
...
The story above brings forth well-known issues of popular evaluation procedures for machine-learning classifiers:
\begin{enumerate}[label=(\arabic*)]
\item We are faced by an avalanche of possible evaluation metrics to use. Often it is not clear which is most compelling. In the story above one could argue that the true-positive rate was the appropriate metric, in view of the great difference in the consequences of correct or wrong classification of class 0, as compared with that of class 1. But at which point does this qualitative reasoning fail? Imagine for example that the net gains and losses had been as follows instead:
\begin{equation}
  ...
  \rotatebox[origin=c]{90}{
    ...
    \overbracket[0pt]{
      \begin{bmatrix}
        340\,\text{\texteuro} & 210\,\text{\texteuro}  \\
        -660\,\text{\texteuro} & 200\,\text{\texteuro}
      \end{bmatrix}}^{
      \textit{\parbox{\widthof{\scriptsize true class}}{\centering\scriptsize true class\\$0\hspace{3em}1$}}
    } \ .
\end{equation}
One could argue that there is still a great difference in the consequences of correct or wrong classification of class 0, as compared with that of class 1, and therefore conclude that the true-positive rate is still the appropriate metric. Yet, a simple calculation shows that classifier B actually leads to the best average revenue in this case: $5.0\,\text{\texteuro/component}$, vs only $3.8\,\text{\texteuro/component}$ for classifier A. So the true-positive rate would \emph{not} be the appropriate metric.
...
\item Most popular metrics were introduced by intuitive reasoning, ad hoc mathematical operations, special assumptions (such as gaussianity), and analysis of special cases \autocites[e.g.][around p.~371 for the $F_{1}$-measure]{rijsbergen1974}{matthews1975}[\sect~31 p.~183 for the Matthews correlation coefficient]{fisher1925_r1963}. Unfortunately such derivations do not guarantee generalization to all cases, nor that the derived metric is uniquely determined by basic requirements -- compare instead, for instance, the derivation of the Shannon entropy \autocites{shannon1948}[\sect~3.2]{woodward1953_r1964} as the \emph{unique} metric satisfying a general set of basic requirements for the amount of information.
  
\item Suppose that some of the popular metrics identify the best algorithm \enquote{in the majority of cases} -- although it is difficult to statistically define such a majority, and no real statistics have ever been collected for this purpose. Yet, do we expect the end user to simply hope not to fall among the unlucky minority? Is this acceptable? Imagine a story analogous to the one above but in a medical setting. The classes represent tumour types, requiring two different types of intervention. The confusion matrices are the same. Instead of monetary gain or loss, correct or incorrect classification leads to the following expected remaining life lengths for patients in a specific age range:
\begin{equation}
  ...
  \rotatebox[origin=c]{90}{
    ...
    \overbracket[0pt]{
      \begin{bmatrix}
        510\,\text{months} & 460\,\text{months}  \\
        10\,\text{months} & 470\,\text{months}
      \end{bmatrix}}^{
      \textit{\parbox{\widthof{\scriptsize true class}}{\centering\scriptsize true class\\$0\hspace{5em}1$}}
    } \ .
  \end{equation}
  This matrix is equivalent to~\eqref{eq:utility_example} up to a common shift and rescaling of all elements. The value 0 means immediate death. It is easy to see that the metrics are exactly as before, the majority favouring classifier~B. And yet the use of classifier~A leads to a slightly longer expected remaining life than classifier~B.

\item Often it is not possible to deploy all candidate classifiers to observe which factually leads to the best results. Or it may even be unethical: consider situation as the medical one above, where some alternatives may lead to immediate deaths.


  
\item What is the rationale behind the mathematical form of the different metrics?
