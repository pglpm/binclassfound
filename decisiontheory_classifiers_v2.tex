...
%% Last-Updated: 2022-05-16T22:02:47+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The second phenomenon is illustrated in \figs~\ref{fig:metrics_vs_utility}--\ref{fig:metrics_vs_utility2}, which we now explain.
...
\end{enumerate}
and we plot a corresponding point with these two numbers as $x$- and $y$-coordinates.
This plot thus shows the general statistical relation between the appropriate utility score and the $F_{1}$-measure of this distribution of confusion matrices.
...
In all these plots we see that it is  always possible to find two or more confusion matrices (yellow non-triangular pairs) for which the $F_{1}$-measure or the Matthews Correlation Coefficient give completely reversed rankings, scoring the worst confusion matrix (and the algorithm yielding it) as the best, and the best as the worst. It is even possible to find at least two confusion matrices (red triangular pairs) for which both these metrics simultaneously give the wrong ranking. It turns out that \emph{there is no classification problem for which these wrong-classification cases do not occur}. This is the second phenomenon mentioned above.
...
\begin{figure}[p]
  \centering
  %{A}\hfill\mbox{}\\[0pt]
\includegraphics[width=0.95\linewidth]{utility_vs_metrics_0.5_1001.jpg}\\[0pt]
% {B}\hfill\mbox{}\\[0pt]
\vspace{-1em}{\color{mygrey}\rule{0.66\linewidth}{0.25pt}}
\includegraphics[width=0.95\linewidth]{utility_vs_metrics_0.8_1001.jpg}\\[0pt]
\caption{Relationship between various metrics and actual utility yield for a binary-classification problem with underlying utility matrix $ \begin{bsmallmatrix} 1&0\\0&1 \end{bsmallmatrix} $. Upper panel: balanced dataset, either class with $50\%$ frequency. Lower panel: imbalanced dataset, class~$0$  with $80\%$ frequency.
  Pairs of yellow non-triangular shapes in a plot show two confusion matrices which are wrongly ranked by the metric of the plot's $y$-axis.   Clearly there can be even three or more confusion matrices ranked in completely opposite order by the metric.
  The accuracy always give a correct rank, no matter the class balance. It is indeed the metric corresponding to the specific utility matrix of this classification problem.
  Red triangular pairs (almost overlapping in some plots) show two confusion matrices which are wrongly ranked by \emph{all} metrics simultaneously except the accuracy.}
...
\begin{figure}[p]
  \centering
  %{A}\hfill\mbox{}\\[0pt]
\includegraphics[width=0.99\linewidth]{utility_vs_metrics_0.5_1000.jpg}\\[0pt]
% {B}\hfill\mbox{}\\[0pt]
\vspace{-1em}{\color{mygrey}\rule{0.66\linewidth}{0.25pt}}
\includegraphics[width=0.99\linewidth]{utility_vs_metrics_0.8_1000.jpg}\\
  \caption{As for \fig~\ref{fig:metrics_vs_utility} but for a binary-classification problem with underlying utility matrix $ \begin{bsmallmatrix} 1&0\\0&0 \end{bsmallmatrix} $. The true-positive rate (recall) is the metric corresponding to this utility matrix, and therefore always leads to correct ranking, no matter the class balance.}
