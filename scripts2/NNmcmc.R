...
## Last-Updated: 2022-06-01T15:14:57+0200
################
...
maincov <- 'class'
family <- 'Palatino'
## ndata <- 8192L # set this if you want to use fewer data
...
variateinfo <- data.table(
    variate=c('class', 'output0', 'output1'),
    type=c('binary', 'double', 'double'),
    min=c(0, -Inf, -Inf),
    max=c(1, +Inf, +Inf),
    mean_mean=c(NA, 0, 0),
    mean_sigma=c(NA, 10, 10),
    sigma_sqrtrate=c(NA, 1, 1),
    sigma_shape=c(NA, 1/4, 1/4) # with 1/4, then min SD value is ~ 0.3 times the one in sigma_sqrtrate
)

#### FILE WITH CALIBRATION DATA
datafile <- 'NN_calibrationdata.csv'
...
##
## load functions for post-MCMC calculations
...
##
## PRIOR PARAMETERS
## this is for integer-valued variates
if(nicovs > 0){
    maxicovs <- apply(alldata[1:ndata,..integerCovs],2,function(x)max(x, na.rm=T))
    thmaxicovs <- covMaxs[integerCovs]
    matrixprobicovs <- matrix(0, nrow=nicovs, ncol=max(thmaxicovs), dimnames=list(integerCovs))
