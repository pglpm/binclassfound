...
## Last-Updated: 2022-06-01T15:13:57+0200
################
...
variateinfo <- data.table(
    variate=c('class', 'output1'),
    type=c('binary', 'double'),
    min=c(0, 0),
    max=c(1, 1),
    mean_mean=c(NA, 0.5),
    mean_sigma=c(NA, 1),
    sigma_sqrtrate=c(NA, 0.01/0.3),
    sigma_shape=c(NA, 1/4) # with 1/4, then min SD value is ~ 0.3 times the one in sigma_sqrtrate
)

#### FILE WITH CALIBRATION DATA
datafile <- 'RF_calibrationdata.csv'
