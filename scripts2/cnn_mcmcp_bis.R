...
## Last-Updated: 2022-05-28T11:16:28+0200
################
...
datafile <- 'modCHEMBL205_predictions_CNN_test1_calibration.csv'
...
#        q=rep(1/nclusters, nclusters) # variables
    ...
    if(nrcovs>0){# real variates
        list(# hyperparameters
            meanRmean0=medianrcovs*0+0,
            meanRtau0=1/(widthrcovs*0+10)^2, # dims = inv. variance
...
for(stage in stagestart+(0:nstages)){
    ...
    for(avar in covNames){
        if(avar %in% realCovs){
            ...
            Ygrid <- X2Y[[avar]](Xgrid)
            jacobians <- Xjacobian[[avar]](Xgrid)
        }else{
            datum <- alldata[1:ndata][[avar]]
            rg <- range(datum, na.rm=T)
            rg <- round(c((covMins[avar]+7*rg[1])/8, (covMaxs[avar]+7*rg[2])/8))
            Ygrid <- Xgrid <- rg[1]:rg[2]
            jacobians <- 1
        }
        Ygrid <- cbind(Ygrid)
        colnames(Ygrid) <- avar
        plotsamples <- jacobians * samplesF(Y=Ygrid, parmList=parmList, nfsamples=min(64,nrow(mcsamples)), inorder=FALSE)
