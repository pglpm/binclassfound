...
%% Last-Updated: 2022-05-30T03:01:01+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\mynotew{explain that if output is already a probability, the transducer will asymptotically reproduce it.}
...
\section{Summary, discussion, and teasers of further ...}
...
De~Finetti's theorem states that the probability density of a value $d_{0}$ for a new unit \enquote{0} is given by the following integral:
\begin{equation}
  \label{eq:prob_is_expe_freq}
  \p(d_{0}) = \int\! F(d_{0})\ \wf(F)\ \di F \ ,
\end{equation}
which can be given an intuitive interpretation. We consider every possible long-run frequency distribution $F(d)$ of data; give it a weight $\wf(F)$; and then take the weighted sum of all such long-run frequency distributions.
...
F(D) \equiv F(d_{1})\cdot F(d_{2})\cdot \dotsm\cdot F(d_{M}) \ .
  \end{equation}
...
The weight $\wf(F)$  tells us how much we can expect the long-run frequency distribution $F$ to change, given new data.
...
The use of decision theory requires sensible probabilities for the possible classes, which brings us to issue*** above. In the present work we also present and use a computationally inexpensive way of calculating these probabilities from the ordinary output of a \ml\ classifier, both for classifiers such as \mynotez{example here} that can only output a class label, and for classifiers that can output some sort of continuous score.
...
In \sect~\ref{sec:prob_calc} we present some notation and the general procedure for the calculation of the probabilities; more technical details are given in appendix***. Section*** explains how to augment a classifier's output with the probability calculation. Results of numerical experiments are presented in \sect***.
