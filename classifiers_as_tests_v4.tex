...
%% Last-Updated: 2022-05-31T09:26:53+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}\labelsep 0pt%
  \noindent
  In a classification task from a set of features, one would ideally like to have the probability of the class conditional on the values of the features. Such probability computationally almost impossible to find in many important cases. The primary idea of the present work is to calculate the probability of a class conditional not on the features, but on a trained classifying algorithm's output. Such probability can be easily calculated, in a model-free and exact way, for any algorithm with a low-dimensional output. This calculation needs to be done only once and provides an output-to-probability \enquote{transducer} that can afterwards be applied to the algorithm's outputs in new classification instances. The probabilities thus obtained can be used, in conjunction with problem-dependent utilities, to perform the classification or even choose among a set of more general decisions, by means of expected-utility maximization. The combined procedure is a computationally cheap \enquote{augmentation} of the original classifier's capabilities. This idea is demonstrated with a simplified drug-discovery problem with a highly imbalanced dataset. The augmentation leads to improved results in all cases, under all possible combinations of problem-dependent utilities. The calculation of the transducer also provides, automatically: (i) a quantification of the uncertainty about the transducer itself; (ii) the expected utility of the augmented algorithm (including its uncertainty), which can be used for algorithm selection; (iii) the possibility of using the algorithm in a \enquote{generative mode}, providing the probabilities of the output given the class, which are useful if the training dataset is biased. It is argued that the optimality, flexibility, and uncertainty assessment provided by the transducer and augmentation methods are dearly needed for classification problems in important fields such as medicine and drug discovery.
