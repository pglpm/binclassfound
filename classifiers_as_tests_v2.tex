...
%% Last-Updated: 2022-05-29T16:46:02+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We consider each of these utility matrices, in turn, to be the one underlying our classification problem. In each case we perform the classification of every item -- a molecule -- in the demonstration data as follows, according to the utility-maximization procedure explained in \sect~\ref{sec:utility_classification}:
...
The standard method produces the same confusion matrix in all four utility cases because it does not use utilities to choose a class. The method with the transducer produces instead a different confusion matrix in each utility case: even if the class probabilities for a give datum are the same in all cases, the threshold of acceptance varies so as to always be optimal for the utilities involved.
...
We can finally assess the performance of both classifiers, with or without the use of the transducer and utility-maximization, on the demonstration dataset. As explained in our companion work \autocites{dyrlandetal2022} and summarized in \sect~\ref{sec:utility_classification}, the correct metric for such performance must naturally depend on the utilities that underlie the problem. It is the utility yield per datum produced by the classifier on the dataset, obtained by taking the grand sum of the products of the homologous elements of the utility matrix $(U_{ij})$ and the confusion matrix $(C_{ij})$:
\begin{equation}
  \label{eq:performance_metric}
  \sum_{ij} U_{ij}\ C_{ij} \ .
\end{equation}

The utility yields for the different cases, classifiers, and methods are presented in table~\ref{tab:results_utilities}.
...
\begin{table}[!p]
  \centering
  \small
  \begin{tabular*}{\linewidth}{ccw{c}{0.12\linewidth}w{c}{0.12\linewidth}w{c}{0.12\linewidth}w{c}{0.12\linewidth}}
&    &$\sumatrix{1}{0}{0}{1}$
 &$\sumatrix{1}{-10}{0}{10}$
...
% ##            0.620  26.100   73.460   1.530    2.24
 &\parbox{0.21\linewidth}{\color{myred}\clap{standard method}}
 &\multicolumn{4}{c}{\textcolor{myred}{$\sumatrix{3225}{79.5}{37}{246.5}$}}
 \\[2\jot]
 \rotatebox[origin=l]{90}{\quad\clap{\parbox{5em}{\centering Random\\Forest}}}
 &\parbox{0.21\linewidth}{\color{mypurpleblue}transducer \amp\\max utility}
...
% ##            0.310   8.370   32.530   5.370    6.23
 ...
 \\[2\jot]
 \rotatebox[origin=l]{90}{\quad\clap{\parbox{8em}{\centering Neural\\Network}}}
 &\parbox{0.21\linewidth}{\color{mypurpleblue}transducer \amp\\max utility}
...
% ##            0.620  26.100   73.460   1.530    2.24
 ...
 %& \textcolor{myred}{88.9}
 \\[1\jot]
 \rotatebox[origin=l]{90}{\ \clap{\parbox{5em}{\centering Random\\Forest}}}
 &\parbox{0.21\linewidth}{\color{mypurpleblue}transducer \amp\\max utility}
 &\textcolor{mypurpleblue}{0.974}
 &\textcolor{mypurpleblue}{1.72}
 &\textcolor{mypurpleblue}{9.63}
 &\textcolor{mypurpleblue}{9.09}
...
% ##            0.310   8.370   32.530   5.370    6.23
 ...
 %&\textcolor{myred}{85.6}
 \\[1\jot]
 \rotatebox[origin=l]{90}{\ \clap{\parbox{8em}{\centering Neural\\Network}}}
 &\parbox{0.21\linewidth}{\color{mypurpleblue}transducer \amp\\max utility}
 &\textcolor{mypurpleblue}{0.962}
 &\textcolor{mypurpleblue}{1.64}
