...
%% Last-Updated: 2022-05-22T11:11:39+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\newcommand*{\dI}{\textit{I}}
\newcommand*{\dII}{\textit{II}}
\newcommand*{\tA}{\textit{A}}
\newcommand*{\tB}{\textit{B}}
\newcommand*{\tC}{\textit{C}}
...
As the potential of using \ml\ algorithms in important fields such as medicine increases, the \ml\ community ought to keep in mind what the actual needs and goals in such fields are. We must avoid trying to (consciously or unconsciously) convince such fields to change their needs just to fit \ml\ solutions that are available and fashionable at the moment. Rather, we must make sure the available solutions fit those needs, and amend them if they do not.

For example, imagine you're a clinician and must take care of a patient with a particular disease. The disease may appear in two variants \dI\ and \dII, and you are not sure which type affects your patient. For this disease there are three kinds of treatment \tA, \tB, \tC\ available at present, and you must choose one of them. Their efficacies, measured on some scale, depend on the disease type according to the following table:
...
\begin{equation*}
  ...
  \overbracket[0pt]{
    ...
    2 & -1 \\ 1 & 1 \\ -1 & 2
      \end{bmatrix}}^{
      \mathclap{\text{\parbox{8em}{\centering\scriptsize disease type\\\dI\hspace{2.5em}\dII}}
    }}
\end{equation*}
Treatment \tA\ is very effective for disease type \dI\ but causes some amount of harm (hence the negative value) if administered to a patient with disease type \dII; vice versa for treatment $\tC$. Treatment \tB\ does not cause harm but is less effective on either disease type.

Which treatment do you choose?

The answer of course depends on the patient's disease 
