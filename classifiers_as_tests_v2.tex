...
%% Last-Updated: 2022-05-29T20:26:31+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The utility yields for the different cases, classifiers, and methods are presented in table~\ref{tab:results_utilities}. The minimum and maximum theoretically achievable yields (all data correctly classified or wholly misclassified) are also shown for each case.

Let us first compare the two algorithms when employed in the standard way (red). Their performance is very close to the theoretical maximum in most cases  The \RF\ outperforms the \CNN\ in utility cases~I and~II; vice versa in cases~II and~III.

Then let us look at the performances obtained with the augmentation (blue bold). We notice the following:
\begin{itemize}
\item The augmentation always improves the performance of each algorithm; in some cases, such as~III, by a large relative amount. The improvement occurs also in cases, such as case~I, where the standard method already had an almost optimal performance.
\item In three cases, the augmentation improves the originally worse algorithm above the originally better one. This almost happens also in case~I, the difference possibly being at the level of statistical variability.
\end{itemize}
