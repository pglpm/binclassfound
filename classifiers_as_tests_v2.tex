...
%% Last-Updated: 2022-05-24T20:05:22+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Effecting the probability transduction: discriminative and generative modes}
...
We show a concrete example of implementation and use of the probability transducer for two classifying algorithms on one data set. The algorithms are a convolutional neural network and a random forest. The data come from the ChEMBL database \autocites{chembl2014}.
...
The data comes from the open-access CheMBL bioactivity database \autocites{chembl2014}. The dataset that is used in this paper was introduced in the paper by  \textcite{koutsoukasetal2017}. The dataset is a source of structure-activity relationship from version 20 of ChEMBL with protein target Carbonic Anhydrase II (ChEMBL205). 
...
For our convolutional neural network, the data is represented by converting the molecule into images in 224\texttimes 224 size. This is done by using the SMILES string from the dataset and using RdKit \autocites{rdkit2017} to generate a canonical graph structure of the molecule. This differs from ECFP in that it represents the actual structure of the molecule rather than properties generated from the molecule. 
...
Virtual screening is the process of finding chemical activity in the interaction between a compound (molecule) and a target (protein). The goal of the machine learning algorithms is to find structural features or chemical properties that show that the molecule is active against the protein. Deep neural networks have previously been shown to outperform random forests and various linear models in virtual high-throughput screening and quantitative structure-activity relationship (QSAR) problems \autocite{koutsoukasetal2017}. 
