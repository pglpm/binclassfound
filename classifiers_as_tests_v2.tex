...
%% Last-Updated: 2022-05-25T17:23:58+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We show a concrete example of implementation of the probability transducer and its use for utility-based classification, for a binary-classification task, two \ml\ classifiers, and one data set.

The classification task is a simplified version of an early-stage drug-discovery problem: to determine whether a molecule is chemically \enquote{inactive} (class~$0$) or \enquote{active} (class~$1$) towards one specific target protein.
...
The joint probability of class $c$ and output $y$, \eqn~\eqref{eq:representation_P}, for the random forest is more specifically given by
\begin{equation}
  \label{eq:P_c_out_RF}
  p(c, y) = \sum_{k} q_{k}\
  [c\ alpha + (1-c)\ (1-\alpha)]\
...
\end{equation}
where $\No$ is a gaussian, \eqn~\eqref{eq:distr_output}, $L(y)$ is a logit transformation (together with a small contraction around $0.5$ to take care of $0$ and $1$ values) that maps the output $y\in \clcl{0,1}$ onto the real numbers \autocites[\cf][\sect~3.3 esp.\ \eqn~(19)]{johnson1949}, and $L'(y)$ its Jacobian determinant. 
