...
%% Last-Updated: 2022-05-23T09:41:46+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item\label{item:determ_statist} The relation from image pixels to house-pet subject may be almost deterministic; so we are effectively extrapolating or looking for a function $\text{\small pet} \mo f(\text{\small pixels})$ contaminated by little noise. But the relation between medical-test scores or biochemical features on one side, and disease or drug activity on the other, is typically \emph{probabilistic}; so a function $\text{\small disease} \mo f(\text{\small scores})$
  % scores${}\!\mapsto\!{}$disease
  or $\text{\small activity} \mo f(\text{\small features})$
  % features${}\!\mapsto\!{}$activity
  does not even exist. \emph{We are assessing statistical relationships} $\P(\text{\small disease} , \text{\small scores})$ or $\P(\text{\small activity} , \text{\small features})$ instead, which include deterministic ones as special cases.
...
\[\P(\text{\small class}, \text{\small feature} \| \text{\small training data}) \ ,\]
...
In order to explain the main idea, let us consider first the essentials behind a classification (or regression) problem. We have the following quantities:
\begin{itemize}
\item the \emph{feature} values of a set of known units,
\item the \emph{classes} of the same set of units,
\end{itemize}
which together form our \emph{training data},
\begin{itemize}[resume]
\item the feature value of a new unit.
\end{itemize}
From these we would like to infer the class of the new unit. This inference consists in probability values
\begin{equation}
  \label{eq:prob_essential}
  \P(\text{\small class of new unit} \|
  \text{\small feature of new unit}, \text{\small training data})
\end{equation}
for each possible class, obtained through the rules of the probability calculus \autocites{jaynes1994_r2003}[\chaps~12--13]{russelletal1995_r2022,gregory2005,hailperin2011,jeffreys1939_r1983}. Combined with a set of utilities, these probabilities allow us to determine an optimal, further decision to be made among a set of alternatives. Note that the inference~\eqref{eq:prob_essential} includes deterministic interpolation, that is the assessment of a function $\text{\small class} \mo f(\text{\small feature})$, as a special case, when the probabilities are essentially $0$s and $1$s.

A trained classifier should ideally yield the probabilities above. Machine-learning classifiers trade this capability for computational speed -- with an increase in the latter of several orders of magnitude \autocites[to understand this trade-off in the case of neural-network classifiers see \eg][]{mackay1992,mackay1992b}[\sect~16.5, esp.~16.5.7]{murphy2012}.
