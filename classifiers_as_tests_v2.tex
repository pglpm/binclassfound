...
%% Last-Updated: 2022-05-28T23:48:23+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Technical details about the setup and training of the two classifiers and of the calculation of the probability-transducer parameters are given in appendix\mynotew{\ldots}.


\mynotew{move this to technical appendix}
The Monte Carlo calculation of the parameters for the transducer took approximately 45\,min for the \RF\ and 75\,min for the \CNN, wall-clock time (16 parallel cores each). The resulting parameters are available in our supplementary data\footcites[\addcomma\ files \texttt{transducer\_params-Random\_Forest.zip} and \texttt{transducer\_params-Neural\_Net.zip}]{dyrlandetal2022c}.
...
\begin{equation}
  ...
  \umatrix{1}{0}{0}{1} \ ,\quad
  \umatrix{1}{-10}{0}{10} \ ,\quad
  \umatrix{1}{-100}{0}{100} \ ,\quad
  \umatrix{10}{0}{-10}{1} \ .
\end{equation}
The first represents any case where the correct classification of either class is equally valuable; and the incorrect classification, equally invaluable. Note that this utility matrix is equivalent to any other of the form $\sumatrix{a}{-b}{-b}{a}$ with $a,b \ge 0$ and $a \ne b$. Accuracy is the correct metric to evaluate this case. The second represents any case where the correct classification of class~1, \enquote{active}, is ten times more valuable than that of class~0 \enquote{inactive}, and its incorrect classification is as damaging as correct classification is valuable. The remaining two matrices are interpreted in an analogous way. The \enquote{value} could simply be the final average monetary revenue at the end of the drug-discovery project that typically follows any of these four situations. Of particular relevance to drug discovery is the second and maybe the third utility matrix, as it is known that false positives are especially costly \autocites{sinketal2010,hingoranietal2019}.

We consider, in turn, each of these utility matrices to be the one underlying our classification problem, and perform the classification of each item -- a molecule -- in the demonstration data by
\begin{enumerate}[label=\arabic*.]
\item feeding the features of the item to the classifier and registering its output
\item feeding this output to the probability transducer and registering the resulting probability
\item multiplying the resulting probability vector by the utility matrix to determine the expected utility of each class choice
\item choosing the class with higher expected utility (ties have to be decided unsystematically, to avoid biased results).
\end{enumerate}
This is done for each classifier.
...
\begin{table}[t]
  \centering
  \small
  \caption{Results for the \rf\ classifier}
...
&    &$\begin{bsmallmatrix*}[r]1&0\\0&1\end{bsmallmatrix*}$
 ...
 &   \textcolor{myred}{standard method}
    ...
    \parbox{\linewidth}{\color{mypurpleblue}transducer \amp\\ utility maximization} &
                                                                                      \textcolor{mypurpleblue}{0.974} & \textcolor{mypurpleblue}{1.72} & \textcolor{mypurpleblue}{9.63} & \textcolor{mypurpleblue}{9.09}
                                                     %&\textcolor{mypurpleblue}{90.9}
    \\[1\jot]
&    \footnotesize relative increase&
                                     \footnotesize+0.6\% & \footnotesize+26\% & \footnotesize+73\% & \footnotesize+1.5\%
                                     % &\footnotesize+2.2\%
    \\[2em]
% standard   0.959   1.52    7.24   8.63   85.6
% transducer 0.962   1.64    9.59   9.09   90.9
%            0.300   8.40   32.50(33)   5.40    6.2
 % \begin{tabular}{p{0.3\linewidth}cccc}
  ...
  %   \\[2\jot]
&    \textcolor{myred}{standard method}&
                                        \textcolor{myred}{0.959} & \textcolor{myred}{1.52} & \textcolor{myred}{7.24} & \textcolor{myred}{8.63}
                                                     %&\textcolor{myred}{85.6}
    \\[0\jot]
     \rotatebox[origin=c]{90}{\clap{\parbox{6em}{\centering Convolutional\\Neural\\Network}}}& 
   \parbox{\linewidth}{\color{mypurpleblue}transducer \amp\\ utility maximization} &
                                                                                      \textcolor{mypurpleblue}{0.962} & \textcolor{mypurpleblue}{1.64} & \textcolor{mypurpleblue}{9.59} & \textcolor{mypurpleblue}{9.09}
                                                     %&\textcolor{mypurpleblue}{90.9}
    \\[1\jot]
&    \footnotesize relative increase&
                                     \footnotesize+0.3\% & \footnotesize+8.4\% & \footnotesize+33\% & \footnotesize+5.4\%
                                                     %&\footnotesize+6.2\%
 \end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{RF_transducer_gainsraw2.pdf}\\
  \caption{Utility yields obtained using the \rf\ classifier in the standard way, vs those obtained using the probability transducer and utility maximization in each single classification, for a uniform distribution of possible utility matrices}
...
%           4.200   2.70    0.9  15.20    0.7
  ...
  \begin{tabular}{p{0.3\linewidth}ccccc}
    ...
    \\[2\jot]
    \textcolor{myred}{standard method}&
\textcolor{myred}{0.835} & \textcolor{myred}{3.75} & \textcolor{myred}{34.5} & \textcolor{myred}{3.80} & \textcolor{myred}{33.4}
    \\[0\jot]
    \parbox{\linewidth}{\color{mypurpleblue}transducer,\\ generative mode} &
\textcolor{mypurpleblue}{0.957} & \textcolor{mypurpleblue}{6.87} & \textcolor{mypurpleblue}{66.7} & \textcolor{mypurpleblue}{3.84} & \textcolor{mypurpleblue}{33.6}
    \\[1\jot]
    \footnotesize relative increase&
                       \footnotesize+15\% & \footnotesize+83\% & \footnotesize+93\% & \footnotesize+1.1\% & \footnotesize+0.4\%
    \\[1\jot]
    \parbox{\linewidth}{\color{myyellow}\scriptsize transducer,\\ discriminative mode} &
\textcolor{myyellow}{\scriptsize 0.918} & \textcolor{myyellow}{\scriptsize 6.68} & \textcolor{myyellow}{\scriptsize 66.1} & \textcolor{myyellow}{\scriptsize 3.33} & \textcolor{myyellow}{\scriptsize 33.3}
  \end{tabular}
...
\bigskip
  ...
  \centering
  \small
  \caption{Results for the \cnn\ classifier}
  \label{tab:results_5_CNN_gen}
% standard 0.894   4.99   47.0   3.78   32.7
% discr    0.861   6.47   66.5   3.33   33.3
% gener    0.930   6.80   66.7   3.74   33.4
%          4.000  36.30   42.0  -0.90    2.1
%          8.000   5.20    0.3  12.30    0.1
  \begin{tabular}{p{0.3\linewidth}ccccc}
    &$\begin{bsmallmatrix*}[r]1&0\\0&1\end{bsmallmatrix*}$
                          &$\begin{bsmallmatrix*}[r]1&-10\\0&10\end{bsmallmatrix*}$
                          &$\begin{bsmallmatrix*}[r]1&-100\\0&100\end{bsmallmatrix*}$
    ...
    \\[2\jot]
    \textcolor{myred}{standard method}&
                                        \textcolor{myred}{0.894} & \textcolor{myred}{4.99} & \textcolor{myred}{47.0} & \textcolor{myred}{3.78} & \textcolor{myred}{32.7}
    \\[0\jot]
    \parbox{\linewidth}{\color{mypurpleblue}transducer,\\ generative mode} &
\textcolor{mypurpleblue}{0.930} & \textcolor{mypurpleblue}{6.80} & \textcolor{mypurpleblue}{66.7} & \textcolor{mypurpleblue}{3.74} & \textcolor{mypurpleblue}{33.4}
    \\[1\jot]
    \footnotesize relative increase&
                       \footnotesize+4.0\% & \footnotesize+36\% & \footnotesize+42\% & \footnotesize-0.9\% & \footnotesize+0.1\%
    \\[1\jot]
    \parbox{\linewidth}{\color{myyellow}\scriptsize transducer,\\ discriminative mode} &
\textcolor{myyellow}{\scriptsize 0.861} & \textcolor{myyellow}{\scriptsize 6.47} & \textcolor{myyellow}{\scriptsize 66.5} & \textcolor{myyellow}{\scriptsize 3.33} & \textcolor{myyellow}{\scriptsize 33.3}
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{RF_transducer_gains_generraw2.pdf}\\
